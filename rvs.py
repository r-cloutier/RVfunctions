...
def stellar_density(Rp, Rs, b, T, P):
    G = 6.67e-11
    Rp, Rs, T, P = Rearth2m(Rp), Rsun2m(Rs), days2sec(T), days2sec(P)
    depth = (Rp/Rs)**2
    rho = 4*np.pi**2 / (P*P*G) * (((1+np.sqrt(depth))**2 - b*b*(1-np.sin(np.pi*T/P)**2)) / (np.sin(np.pi*T/P)**2))**(1.5)  # kg/m3
    return rho * kg2Msun(1) / m2Rsun(1)**3
